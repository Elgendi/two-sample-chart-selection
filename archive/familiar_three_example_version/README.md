...
- Figure 1: how the score is calculated, with actual candidate scores and worked arithmetic.
- Figure 2: different winners: a violin (85.67 to 98.01) and a histogram (85.41 to 97.49).
- Figure 3: unchanged mean bars (95.35) and the winner distribution across all 127 comparisons.

Read `reader_figures/READ_THE_FIGURES.md` for a plain-language explanation and commands for your own data. `score_my_data.py` evaluates one two-column CSV and retains tied optima. The supplied example reproduces Figure 1.
...
The three main figure sources, PDF/SVG/PNG exports, exact values and checks are in `reader_figures/`. The original technical figures are retained in the supplement. The underlying v3 benchmark has not been changed to create different winners.
