...
key='BIDMC__01';c='Dot plot / raw';best=score(key,c);target,read,err,scale=recovery(key,c)
fig=plt.figure(figsize=(9,10.4));header(fig,'01 / How the score works','Let the data choose the chart.','Example: patient-level median rates from ECG and a pulse sensor')
...
chart(fig,[.59,.470,.35,.183],key,c,units='Rate (beats/min)')
txt(fig,.57,.402,'Each dot represents one patient.\nBest among 21 tested settings.',fontsize=10,color=GREY,linespacing=1.55)
...
RECORDS.append(dict(figure=1,key=key,candidate=c,score=best,loss=loss,mean_absolute_error=err.mean(),scale=scale));save(fig,'Figure_1_How_scoring_works')
# Two actual datasets, same task and candidates, different selected families.
fig=plt.figure(figsize=(10,11));header(fig,'02 / Different data, different choices','The best chart depends on the data.','Same question • same seven families • same score • 21 settings per comparison')
examples=[('Cleveland__03','Violin plot / 64','A  Maximum heart rate','Beats/min',.49),('HeartFailure__01','Histogram / 8','B  Blood enzyme concentration','Units/L',.08)]
for key,c,title,units,y in examples:
 before=score(key,BAR);after=score(key,c);label(fig,.06,y+.32,title)
 txt(fig,.06,y+.279,f'BEFORE  Mean bars   {before:.2f}',fontsize=14,weight='bold',color=GREY)
 txt(fig,.55,y+.279,f'AFTER  {c.split(" / ")[0]}   {after:.2f}',fontsize=14,weight='bold',color=TEAL)
 chart(fig,[.10,y+.055,.34,.175],key,BAR,units=units)
 chart(fig,[.59,y+.055,.34,.175],key,c,units=units)
 txt(fig,.55,y+.004,f'+{after-before:.2f} score points',fontsize=13,weight='bold',color=TEAL)
 RECORDS.append(dict(figure=2,key=key,candidate=c,before_score=before,score=after,gain=after-before))
txt(fig,.06,.021,'Examples illustrate computed winners; they do not estimate the average benefit.',fontsize=10,color=GREY)
save(fig,'Figure_2_When_optimization_helps')
# Honest no-replacement case plus the complete winner distribution.
key='Wrist__00';best=score(key,BAR)
fig=plt.figure(figsize=(9,10));header(fig,'03 / When keeping the chart is best','Optimization can mean: keep the chart.','Example: early versus late exercise heart rate in eight participants')
label(fig,.06,.816,'START  Mean bars');label(fig,.56,.816,'SELECTED  Mean bars')
for x in [.06,.56]:txt(fig,x,.766,f'{best:.2f}',fontsize=31,weight='bold',color=TEAL)
chart(fig,[.09,.51,.36,.20],key,BAR,units='Rate (beats/min)');chart(fig,[.59,.51,.36,.20],key,BAR,units='Rate (beats/min)')
box(fig,[.06,.417,.88,.052]);txt(fig,.079,.435,'Change: 0.00 points. None of the tested alternatives scores higher.',fontsize=11,weight='bold')
label(fig,.06,.369,'Across all 127 comparisons, no one family always wins')
w=A.sort_values('loss').groupby('key').head(1);counts=w.family.value_counts().reindex(['Heatmap','Dot plot','Histogram','Violin plot','Bar chart','Box plot','Interval plot'],fill_value=0)
ax=fig.add_axes([.23,.103,.65,.222]);ys=np.arange(7);ax.barh(ys,counts,color=TEAL,height=.64);ax.set_yticks(ys);ax.set_yticklabels(counts.index,fontsize=10);ax.invert_yaxis();ax.set_xlim(0,65);ax.set_xticks([0,20,40,60]);ax.set_xlabel('Number of comparisons with the best score',fontsize=10);ax.tick_params(length=0)
for y,v in zip(ys,counts):ax.text(v+1,y,str(v),va='center',fontsize=11)
txt(fig,.06,.030,'Winners apply to this task, candidate set and computational observer.\nA high score does not prove that a summary preserves every distributional detail.',fontsize=10,color=GREY)
RECORDS.append(dict(figure=3,key=key,candidate=BAR,score=best,before_score=best,gain=0));save(fig,'Figure_3_When_optimization_does_not_help')
pd.DataFrame(RECORDS).to_csv(OUT/'figure_values.csv',index=False)
A.to_csv(OUT/'familiar_family_scores.csv',index=False);w.to_csv(OUT/'familiar_winners.csv',index=False)
(OUT/'figure_checks.json').write_text(json.dumps(dict(candidate_families=7,settings=21,comparisons=127,winner_counts=counts.to_dict(),source_sha256=hashlib.sha256((ROOT/'validation_v2/results/families.csv').read_bytes()).hexdigest(),examples=RECORDS),indent=2))
pd.DataFrame({'A':pd.Series(CASES['BIDMC__01']['x']),'B':pd.Series(CASES['BIDMC__01']['y'])}).to_csv(OUT/'example_input.csv',index=False)
