...
def main():
 ...
 for family,setting in CANDIDATES:
  ...
  for j,phase in enumerate([0,.25,.5,.75]):
   ...
   try:estimate=targets(decode_all(im,sp,args.observer))[args.task];loss=float(np.mean(abs(estimate-truth))/scale)
